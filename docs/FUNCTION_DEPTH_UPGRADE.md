...
- 活動保存名稱、類型、學期、日期、地點、目標、對象、報名方式、總負責人與備註；資料和使用者、project 綁定。
- 每一項工作保存組別、負責人、期限、優先級與狀態；可直接回答「誰負責什麼」「哪些工作逾期」「活動前還剩什麼」。
- 「這場活動目前缺什麼」同時檢查活動必要欄位、未指派工作、逾期工作與下一個期限，不用模型自行猜。
- 活動與待辦在伺服器重啟後仍存在，而且不能跨使用者讀取或修改。

### 5. 從活動資料延伸產出

- 企劃書、簡報、網宣與會議紀錄會取得同一筆活動的已確認資料與待填欄位。
- 未確認的日期、地點、負責人與報名方式必須保留為待填，不得從歷史檔案補值。
- 活動衍生 artifact 保存 `activity_id`；修改版本仍保留原本的 artifact lineage。
- 沿用同一 project 時自動帶入最近活動，不要求使用者重新描述全部內容。

### 6. 模型成本與工具可靠性政策

- 簡短知識查詢使用快速模型；研究、複合任務及文件產出使用較強模型；使用者指定模型仍優先。
- 任務狀態保存實際選用模型、模型呼叫次數、token 用量、估算成本、工具呼叫與失敗分類。
- 同一輪完全相同的唯讀工具呼叫直接重用結果；會寫資料或產檔的工具永遠不快取。
- 可安全重試的唯讀工具失敗時有限次重試並送出結構化事件；逾時或無法重試時保留失敗步驟與替代路徑。
