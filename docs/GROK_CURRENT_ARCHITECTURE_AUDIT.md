...
| 33 | Existing bugs | HTTP adapter 曾跟隨轉址；`asset_file` 未限制路徑；重啟把 `review_status` 改 failed；Vision 直讀 `storage_path`；搜尋 limit 無上限 | 本輪已修：不跟隨轉址、路徑限制、中斷不改 review_status、分析走 confined 檔案、limit≤100 |
