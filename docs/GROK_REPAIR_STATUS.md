...
- 資料整理 `create_asset` 會帶入 `project_id`；分頁 Query 全部有上限。本機盤點：knowledge 517 md、mobile-shots 12 張 UI 截圖（未當社團相簿匯入）、Windows 素材樹不存在。
- 迴歸：API/session/artifact/frontend/architecture 97 passed；organization+InsForge sync 19 passed；baseline agent/RAG 在同輪未失敗。
