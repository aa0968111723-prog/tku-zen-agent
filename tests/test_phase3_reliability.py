...
async def test_artifact_expected_but_missing_is_blocked(tmp_output_dir, monkeypatch, ctx):
    ...
    assert done, kinds(events)
    assert done[0]["summary"]["workflow_status"] == "blocked"
    assert done[0]["summary"]["status"] == "blocked"
    failed = done[0]["summary"]["failed_steps"]
    assert any("產出" in f["error"] or "檔案" in f["error"] for f in failed)
...
def test_client_ip_uses_xff_only_behind_private_proxy():
    ...
    assert client_ip(_req("10.0.0.2", "203.0.113.5, 10.0.0.2")) == "203.0.113.5"
    # 公網直連帶假 XFF → 不信
